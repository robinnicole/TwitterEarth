...
$( document ).ready( function(){
	...
	window.earth = new THREE.Mesh(
		...
		new THREE.MeshLambertMaterial({ 
			map: THREE.ImageUtils.loadTexture( '../static/media/earthTexture.png' )
		})
	)
	...
	window.clouds = new THREE.Mesh(
		...
		new THREE.MeshLambertMaterial({ 
			map: THREE.ImageUtils.loadTexture( '../static/media/cloudsTexture.png' ),
			transparent: true,
	...
	clouds.position.set( -20, 0, 10 )
	clouds.receiveShadow = true
	clouds.castShadow = true
	earthGroup.add( clouds )
	

var shape = new THREE.TextGeometry("Twitter is for Lovers", {font: 'helvetiker', size: 6, height: 1});
window.words = new THREE.Mesh(shape, new THREE.MeshBasicMaterial( { color:0xcc0000, side: THREE.DoubleSide } ));
...
var loader = new THREE.ColladaLoader();
	loader.load('/media/heart.dae', function (result) {
...
function fetchTweets(){
	...
	$.ajax({
	 ...
	 url: 'http://search.twitter.com/search.json?q=love&geocode=0,0,6400km',  
